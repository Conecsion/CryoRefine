...
def backprojection(projections: torch.Tensor,
    ...
    # Coordinated of rotated central plane
    ...
    # Distance from the nearest neighbor points
    w1 =  (rotated_slice[:, :, :, 0]-floor_x) * (rotated_slice[:, :, :, 1]-floor_y) * (ceil_z-rotated_slice[:, :, :, 2])
    w2 = (rotated_slice[:, :, :, 0]-floor_x) * (ceil_y-rotated_slice[:, :, :, 1]) * (ceil_z-rotated_slice[:, :, :, 2])
    w3 = (ceil_x-rotated_slice[:, :, :, 0]) * (ceil_y-rotated_slice[:, :, :, 1]) * (ceil_z-rotated_slice[:, :, :, 2])
    w4 = (ceil_x-rotated_slice[:, :, :, 0]) * (rotated_slice[:,:,:,1] - floor_y) * (ceil_z-rotated_slice[:, :, :, 2])
    w5 = (rotated_slice[:, :, :, 0]-floor_x) * (rotated_slice[:, :, :, 1]-floor_y) * (rotated_slice[:, :, :, 2]-floor_z)
    w6 = (rotated_slice[:, :, :, 0]-floor_x) * (ceil_y-rotated_slice[:, :, :, 1]) * (rotated_slice[:, :, :, 2]-floor_z)
    w7 = (ceil_x-rotated_slice[:, :, :, 0]) * (ceil_y-rotated_slice[:, :, :, 1]) * (rotated_slice[:, :, :, 2]-floor_z)
    w8 = (ceil_x-rotated_slice[:, :, :, 0]) * (rotated_slice[:, :, :, 1]-floor_y) * (rotated_slice[:, :, :, 2]-floor_z)

    # Initialize Fourier volume
